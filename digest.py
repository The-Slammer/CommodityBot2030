...
Flow:
  1. Fetch 7-day 1h price data for WTI, Natural Gas, CCJ (uranium proxy)
...
COMMODITIES = {
    ...
    "uranium": {
        "label": "Uranium (CCJ Proxy)",
        "unit": "$/share",
        "ticker": "CCJ",
        "av_function": None,          # Use equity time series instead
        "news_tickers": ["CCJ", "UEC", "DNN"],
...
def _fetch_equity_price_series(ticker: str) -> list[dict]:
    """Fetch daily price series for an equity (used for CCJ uranium proxy)."""
    params = {
...
def fetch_price_data() -> dict:
    ...
    for key, config in COMMODITIES.items():
        try:
            ...
            logger.error("Failed to fetch price data for %s: %s", key, e)
            prices[key] = []
    return prices
...
<header>
    <div class="header-top">
        ...
        <span class="header-ticker">WTI · NATGAS · URANIUM · CCJ · XOM · CVX · COP</span>
    </div>
    <div class="masthead">
